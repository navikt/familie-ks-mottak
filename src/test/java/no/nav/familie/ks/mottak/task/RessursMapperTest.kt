...
class RessursMapperTest {
    ...
    fun `map task til ressurs med rest task`() {
        val task = Task.nyTaskMedStartFremITid(JOURNALFØR_SØKNAD, "payload", LocalDateTime.now())
        ...
        assertThat(successRessurs.data?.first()?.get("task")?.get("type")?.textValue()).isEqualTo(JOURNALFØR_SØKNAD)
